...
| key          | 中文名   | 注入方式 | 丢弃的上游说明（`drops`）                                                       | 工具（`tools`）                                                                                                                                         |
| ------------ | -------- | -------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base`       | 基础     | `auto`   | read / write / edit / glob / grep / bash / pwsh / jobs / web_fetch / web_search | read / write / edit / glob / grep / bash（或 pwsh）/ ask_user_question / job_output / job_list / job_kill / read_image / web_fetch / web_search / skill |
| `flow`       | 流程     | 按需     | goal                                                                            | todo_write / exit_plan_mode / get_goal / create_goal / update_goal / present                                                                            |
| `delegation` | 派发     | 按需     | subagent / subagent_fork / workflow                                             | subagent / subagent_fork / list_subagent_models / workflow / list_agents / send_message / interrupt_agent                                               |
| `team`       | 协作编排 | 按需     | team:policy                                                                     | spawn_teammate / wait_agent / team_task_create / team_task_list / team_task_get / team_task_update（成立判据只认这三个入口：`requires`）                |
...
由 [dsh-preset](../../preset/dsh-preset/README.md) 的生成器写进 preset 产物：`standard` 那行不带 config，
`chat` 那行带 `config: { groups: false }`（对话模式不要工具用法）。插件的 Config 只有一个开关 `groups`
（默认 `true`）。
