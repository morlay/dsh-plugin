...
} from "@deepseek-ai/dsh-agent-instructions/src/config.ts";
import { findProjectRoot } from "@deepseek-ai/dsh-agent-instructions/src/files.ts";
import yaml from "js-yaml";
import { describe, expect, it } from "vitest";
import { baselineIdentity } from "../../agent-instructions/baseline.ts";
...
describe("baseline 身份与上游对齐", () => {
  ...
  it("项目根相对 cwd 的位置参与身份（上游语义：工作区形状变了就重新建立基线）", async () => {
    ...
    expect(await baselineIdentity(subdirectory)).not.toBe(await baselineIdentity(process.cwd()));
  });
});
