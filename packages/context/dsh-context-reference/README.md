...
- 本步所有文件合成**一条**注入消息：每个文件一个**内容块** `<file_content path="…">`（行号前缀 +
  续读提示都在信封内），窗口上限（2000 行 / 50 KB / 单行 2000 字符）取 read 的默认值；
  内容块是一次性材料，不参与规则块的覆盖——见[上下文注入规则](../.agents/designs/20260921-上下文注入规则.md)；
...
为什么只认 `@` 起手，见设计[文件引用内容注入](./.agents/designs/20260920-文件引用内容注入.md)。
