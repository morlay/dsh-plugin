...
给模型的注入形态与规则。服务 `packages/context/` 下的包：注入通道、工作区指令、skill 目录、引用展开、
工具用法分组。规则与 id 表见[设计 上下文注入规则](./designs/20260921-上下文注入规则.md)。
...
（[`rules.ts`](../dsh-context-assembler/src/rules.ts)），所以每条 reminder 只带 id 与正文。
