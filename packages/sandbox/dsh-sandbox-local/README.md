...
六个字段都上设置页：`access` 可改（**改完规则当场重算**，不用重挂这一行）；其余五个是**装配事实**，页面上只读
可见——它们在 `Config` 上标了 `.volatile().disabled()`，所以能看清「这一行现在装配成什么样」而改不了。`volatile`
会让解析后的值变成**引用**，而官方 `LocalSandboxProvider` / `LocalFileSystem` 在构造时读一次这些字段并做装配期
校验，所以交给基类的那份配置先由 `upstreamConfigOf` 解包成值（`src/config.ts`）。
