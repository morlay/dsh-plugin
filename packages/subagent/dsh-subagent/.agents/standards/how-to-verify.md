...
本包行为只有一条（回报指引是中文），但**同步纪律**是它的第二半：四个测试文件分别盯一件事。
...
| `__tests__/assembly.spec.ts`              | `cordis.patch.yml` + app 的 bundle 列表        | 上游 `subagent` 行确实被禁用、本包行确实被插入、app 确实引用本包                         |
