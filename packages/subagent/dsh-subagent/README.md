...
上游 `@deepseek-ai/dsh-subagent` 的**薄壳 fork**：只改一件事——continuable 子代理首条任务后面的
...
`subagent-fork`（`@morlay/dsh-subagent`），app 的 `dsh.profile.bundles` 里引用本包。
...
- 决策与理由：[ADR 薄壳 fork 接管 subagent 行只改回报文案](./.agents/adrs/20260923-薄壳fork接管subagent行只改回报文案.md)
