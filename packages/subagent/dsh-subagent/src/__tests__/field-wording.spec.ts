...
describe("子代理 的字段文案", () => {
  ...
  it("注册字典与字段文案", () => {
    ...
    expect(b.dictionaries).toEqual(["settings.subagent"]);
    expect(b.described.map((entry) => ({ ns: entry.ns, path: entry.path }))).toEqual([
      { ns: "subagent-fork", path: ["maxDepth"] },
      { ns: "subagent-fork", path: ["maxActiveSubagents"] },
    ]);
  });
  ...
  it("每个字段的文案来自本包字典", () => {
    ...
    expect(b.described.map((entry) => entry.read())).toEqual([
      { label: zh.maxDepth, hint: zh.maxDepthHint },
      { label: zh.maxActiveSubagents, hint: zh.maxActiveSubagentsHint },
    ]);
  });
