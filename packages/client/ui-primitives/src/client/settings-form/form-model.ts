// 搬自 vendor/deepseek-harness/packages/client/ui-primitives/src/settings-form/form-model.ts
// （上游 `@deepseek-ai/dsh-client-ui-primitives`，见本包 .agents/adrs 里的搬迁记录）；逻辑未改，
// 只有 catch 的未用参数按 lint 口径写成可选绑定。改这里之前先看上游是否也改了同一处。
