...
import { SettingsForm, type SettingsFormShell } from "@morlay/dsh-client-ui-primitives/client";
...
it("discards when it leaves the page, calling the latest discard it was given", () => {
  ...
  const { rerender, unmount } = render(
    <SettingsForm labels={labels} state={settled} onSave={vi.fn()} onDiscard={first}>
      <span />
    </SettingsForm>,
  );
  rerender(
    <SettingsForm labels={labels} state={settled} onSave={vi.fn()} onDiscard={second}>
      <span />
    </SettingsForm>,
  );
