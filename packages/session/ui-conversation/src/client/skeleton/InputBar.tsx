...
export const InputBar = memo(function InputBar({
  ...
  const planActive = useProjection(
    "plan",
    (plan) => plan !== undefined && (plan.pending ? !plan.active : plan.active),
  );
  ...
  const attachments = useMemo(
    () =>
      input === undefined || resolveDraftAttachments === undefined
        ? []
    ...
    (files: readonly File[]): void => {
      ...
      const rejected = ((): string | null => {
        if (imageLimits !== undefined) {
          ...
          const total =
            imageAttachments.reduce((sum, attachment) => sum + attachment.file.size, 0) +
            images.reduce((sum, file) => sum + file.size, 0);
          if (total > imageLimits.maxMessageImageBytes) {
            return t("image.totalTooLarge", {
              size: imageSizeText(imageLimits.maxMessageImageBytes),
            });
          }
        }
  ...
  const plainMessageDraft =
    !empty && input?.phase === "plain" && !draft.trimStart().startsWith("/");
  ...
  const claimActive =
    (input?.phase === "claimed" || input?.phase === "submitting") &&
    input.claim !== undefined &&
    draft.startsWith(input.claim.token);
  const rawHint =
    claimActive &&
    input.claim.hint !== undefined &&
  ...
  return (
    <div {...styling.props(styles.root, variant === "hero" && styles.hero)}>
      {toast !== null && (
        <Toast
          key={toast.seq}
          text={toast.text}
      ...
      {notice?.level === "info" && (
        <div {...styling.props(styles.notice)} role="status">
          {notice.text}
        </div>
      )}
      ...
      <div
        ref={cardRef}
        {...styling.props(styles.card, workspaceTrigger && styles.cardWorkspaceTrigger)}
        data-composer-card
      ...
      >
        {sessionId !== undefined && (
          <div {...styling.props(styles.overlayAnchor)}>
            {renderSlot("conversation.input.overlay", {})}
          </div>
        )}
        {accessory !== undefined && <div {...styling.props(styles.accessory)}>{accessory}</div>}
        ...
        <DraftEditor
          classNames={{
            scroll: styling.className(styles.scroll),
            grow: styling.className(styles.grow),
            input: styling.className(styles.input),
            inputDisabled: styling.className(styles.inputDisabled),
            placeholder: styling.className(styles.placeholder),
          }}
          editor={editor}
          scrollRef={scrollRef}
          editable={editable}
        ...
        <div {...styling.props(styles.row)}>
          <div {...styling.props(styles.tools)}>
            <Tooltip label={t("input.commands")} side="top" delayMs={500}>
              <button
                type="button"
                {...styling.props(styles.add)}
            ...
            />
            <div {...styling.props(styles.modes)}>
              {sessionId === undefined
                ? null
          ...
          <div {...styling.props(styles.trailing)}>
            {input === undefined || sessionId === undefined
              ? null
