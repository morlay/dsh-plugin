...
除键列外各表另带 `f_id` serial 自增主键，例外是五张表：单例表 `t_persistence_state` /
`t_workspace_state`（以 `f_singleton` 为键）、键值表 `t_schema_meta`（以 `f_key` 为键）、
storages 域版本账本 `t_storage_units`（以 `f_name` 为键）与用量日志 `t_event_usage`
（以 `f_event_id` 为键）都不带 `f_id`。**多表关联一律用业务键、
