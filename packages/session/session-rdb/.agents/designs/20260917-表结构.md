...
由 `src/adapters/` 从这些实体转化生成——无手写 DDL；表结构级升级走 `drizzle/` 里的**手写**迁移
（生成器输出与实体定义对不上，见 [债务 20260921-drizzle生成器与实体定义对不上](../debts/20260921-drizzle生成器与实体定义对不上.md)）。
除键列外各表另带 `f_id` serial 自增主键，例外是：单例表 `t_persistence_state` /
`t_workspace_state`（以 `f_singleton` 为键）、键值表 `t_schema_meta`（以 `f_key` 为键）、
storages 域版本账本 `t_storage_units`（以 `f_name` 为键），以及三张派生统计表
（本节末尾「派生统计表」：`t_event_usage` 以 `f_event_id` 为键，两张会话汇总表以业务唯一键为键）。**多表关联一律用业务键、
...
- 表结构级升级由 `drizzle/` 目录里的迁移（sqlite/pg 各一份，**手写**——生成器输出与实体定义对不上，
  见 [债务 20260921-drizzle生成器与实体定义对不上](../debts/20260921-drizzle生成器与实体定义对不上.md)）承担，
  运行时经 drizzle `migrate()` 执行。同版本内
...
| `t_session_events` | `f_event_id`                       | 反向查找（按事件找会话；删会话只删桥接行，孤儿事件行由 GC 回收） |
...
行成为孤儿（事件实体全局共享）。删除会话也**不**清孤儿（跨会话引用不可知，多实例共享数据库时尤甚），
清理统一归 GC 通道（`gc.ts`，全库扫描 + vacuum 分开跑，见
...
- **1:1 的状态不拆表**：会话归档是 `t_sessions.f_archived_at` 标记，钉住是同一行的
  `f_pinned_seq`（非空即已钉住，值就是它在 `pinnedSessionIds` 里的位次；见
  [ADR-跟随上游session-format-v4](../adrs/20260922-跟随上游session-format-v4.md)），两者都与会话行一起查询；投影 checkpoint 的日志 identity 直接复用
...
统计报表（`usageReport`）的读侧维度全部物化在三张衍生表里，表结构与取舍见
[ADR-统计衍生表物化归属与汇总](../adrs/20260923-统计衍生表物化归属与汇总.md)：
**迁移先 DROP 再建**、任一表为空时在打开库时全量回填、随时可清掉重建。

**`t_event_usage`** — 事件级用量（一条 `assistant/message` 事件行一行，fork 共享行不重复）：去重口径的
总量 / subagent 拆分与「天 × 模型」的桶只读它。

| 列                       | 说明                                                               |
| ------------------------ | ------------------------------------------------------------------ |
| `f_event_id`             | 事件行键（外键 → `t_events`，`ON DELETE CASCADE`）                 |
| `f_created_at`           | 事件时间（毫秒；范围过滤按它走，索引在）                           |
| `f_day`                  | host 本地日 `YYYY-MM-DD`（桶分组用，与 `localtime` 口径一致）      |
| `f_provider` / `f_model` | 模型归属（事件里模型报的 provider / model）                        |
| `f_referenced`           | 被任何会话桥接行引用（孤儿行 0，不计入统计）                       |
| `f_subagent`             | 被任何 `origin = 'subagent'` 的会话引用（拆「人类 / 子代理」两组） |
| `f_*_tokens`             | input / output / cacheRead / reasoning / total 五个 token 列       |

**`t_session_usage`** — 会话 × 本地日 × provider × model 的 token 汇总；**按会话的行**读它，
因此保住「fork 子会话含继承前缀」的口径（各会话行之和 ≥ 总量）而不回连事件表。

| 列                       | 说明                                    |
| ------------------------ | --------------------------------------- |
| `f_session_id`           | 会话 id（外键 → `t_sessions`，CASCADE） |
| `f_day`                  | 本地日                                  |
| `f_provider` / `f_model` | 模型归属（未知模型落空串）              |
| `f_*_tokens`             | 五个 token 列（累加）                   |

**`t_session_counts`** — 会话 × 本地日的活动计数：轮次 / 步骤 / 用户输入 / 工具调用四列直接读，
不再存事件类型、也不在读取时折。

- 两张会话汇总表各带 `UNIQUE` 键（`(f_session_id, f_day, f_provider, f_model)` /
  `(f_session_id, f_day)`）、`f_day` 与 `f_session_id` 索引；`ON CONFLICT DO UPDATE` 累加。
- **旁路写**：`appendBatch` 事务提交之后累加两张汇总表（`INSERT ... ON CONFLICT DO UPDATE`），
  不在写事务里、失败只 warn——派生数据，丢几次累加不影响可用性；用量行的物化列在写事务内落
  （新事件行的 `f_referenced` 必为 1）。
- **可销毁重建**：表为空时（首次 / 迁移后 / 手动清空）在打开库时全量回填；rewind / fork 之后按会话重算
  （`rebuildSessionStats`）并**全量重算** `t_event_usage` 的引用标记（`refreshEventUsageFlags`）；
  会话删除时清掉该会话的汇总行，同样重算引用标记（被删会话引用的用量行在 GC 之前也必须退出统计）。
