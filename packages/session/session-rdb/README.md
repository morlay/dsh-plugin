...
| 用途                   | 接缝                                                                             | 内容                                                                                            |
| ---------------------- | -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 官方 UI（上游 `ui-*`） | `session/list` → `ctx.sessionQuery.listSessions`（本包接管，**默认排除已归档**） | 未归档的可见语料，标题走投影（`projections.title`）                                             |
| 我们自己的管理面       | `POST /api/session.rows`（本包路由）                                             | **完整语料（含归档）** + 标题 + 最后活动时间 + 工作区归属，**搜索 / 子代理过滤 / 分页都在后端** |

排除归档是本部署的策略：归档是「收起来」的会话，不该混进日常列表与搜索。管理动作（取消归档 / 删除 /
导入）需要完整集合，所以另给一条自有路由，两条路不混。决策与取舍见
[ADR-会话列表两条路](.agents/adrs/20260922-会话列表两条路.md)。

`session.rows` 的请求体是 `{ query?, includeSubagents?, page?, pageSize? }`（`pageSize` 默认 20、上限 200），
响应是 `{ items, total, page, pageSize }`——**搜索与分页都在后端**：前端分页等于每次拉全量，数据一多就崩。

排序键是会话行的 **`f_last_event_at`**（由写路径维护、老数据由迁移一次性回填、没有事件的会话回落
`f_created_at`）。早先这里对每行算一次「该会话最后事件时间」的相关子查询：真实库 243 行要 **4.2s**，
加 `LIMIT` 也救不了（排序要求全表先算出来）；物化之后同一条分页查询是 **0.2ms**。
...
天 × provider/model × subagent 的桶、按会话的行（活动计数：轮次 / 步骤 / 用户输入 / 工具调用）。

统计读侧只读三张**可销毁重建的派生表**，读侧维度（会话 / 模型 / 本地日 / 是否被引用 / 是否子代理）
全部在写入时物化，查询不做 `EXISTS`、也不回连事件表：

- `t_event_usage`——一条 `assistant/message` 事件行一行（去重口径的总量与「天 × 模型」桶读它）；
- `t_session_usage`——会话 × 本地日 × 模型的 token 汇总（按会话的行读它，含 fork 继承前缀）；
- `t_session_counts`——会话 × 本地日的四项活动计数。

写入时在事务内落用量行、事务后**旁路累加**两张汇总表；rewind / fork / 删除会话之后按会话重算并全量重算
引用标记；表为空（首次 / 迁移后 / 手动清空）在打开库时全量回填——迁移对这三张表先删后建。真实库实测
单次统计从 **1.6s → ~140ms → ~30ms**（口径不变，旧口径逐项交叉核对一致），升级后首次打开付一次回填
（2.4s / 3.09 万用量行，此后不再发生）。口径与取舍见
[ADR-统计衍生表物化归属与汇总](.agents/adrs/20260923-统计衍生表物化归属与汇总.md)。
