...
包内测试（`pnpm exec vitest run packages/preset/dsh-agent-preset`）覆盖行清单与 patch 的一致性、`isolate`
组的完整性、以及 `relax-intent` 的归属判据与 `prepend` 顺序；和真装配两者都要跑，顺序无所谓。

**host 层**（按 id 禁用、llm route、搜索后端、沙箱）的判据在
[`@morlay/dsh-preset` 的规范](../../../dsh-preset/.agents/standards/how-to-verify.md)——那里的动作是全局的，
风险面不同。
