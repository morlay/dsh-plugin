...
个人用 dsh profile bundle：**只注册自定义模式**——`coding` 与 `chat`，各是 `cordis.patch.yml` 里的一行
`@deepseek-ai/dsh-agent-preset`（行的 `config.plugins` 就是该模式的装配清单）。部署级的 host 配置（llm 路由、
搜索后端、沙箱）在 [`@morlay/dsh-preset`](../dsh-preset/README.md)。
