...
export const STANDARD_PERSONA = {
  prefix: [
    ...
    "全程用中文（专有名词除外）；思考不要陷入重复循环，一旦循环立即退出；思考聚焦需求理解与方案设计，不预演具体代码实现，正确性由验证环节确认。",
  ].join("\n"),
...
export const CHAT_PERSONA = {
  prefix: "你是一个助手。全程用中文（专有名词除外）；思考不要陷入重复循环，一旦循环立即退出。",
};
