...
| **context**（上下文注入）                  |                                                                                                                                                             |                                                                      |
| `context/dsh-context-assembler/`           | 注入能力组（一个包五个出口）：通道 `assembler`、工作区指令 `agent-instructions`、skill 目录 `skill-catalog`、工具用法分组 `tool-guidance`、模式收口 `scope` | [README](packages/context/dsh-context-assembler/README.md)           |
...
| **profile**（profile 组成）                |                                                                                                                                                             |                                                                      |
| `profile/dsh-profile/`                     | profile 的配置初始化层：只做 config 覆盖（pi-ai 路由、欢迎提示、搜索后端选择、沙箱规则值），不插行不禁行                                                    | [README](packages/profile/dsh-profile/README.md)                     |
| `profile/dsh-agent-preset/`                | 两个模式的定义：只留提示词（persona）与能力开关，功能行与注入通道从下面两个包引用                                                                           | [README](packages/profile/dsh-agent-preset/README.md)                |
| `profile/dsh-agent-toolkit/`               | agent 功能行清单（工具 / 命令 / 压缩 / 委派 / 工作流）：preset 引用它，profile 也可直接装配                                                                  | [README](packages/profile/dsh-agent-toolkit/README.md)               |
| **sandbox**（沙箱替换）                    |                                                                                                                                                             |                                                                      |
