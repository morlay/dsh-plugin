...
| `profile/dsh-session-mode/`                | 两个会话模式（`coding` / `chat`）作为数据：persona、能力开关、角色（谁能用）与可选默认模型，按会话应用；子代理继承父模式；官方 agent preset 那一套被禁用 | [README](packages/profile/dsh-session-mode/README.md)                |
