...
| 对话 UI 接管（技术债）       | —（无独立词汇）                                                                                                     | 上游对话外壳的**薄壳 fork**（只留我们改过的文件，其余 import 指向上游源码）与其服务包：`session/ui-conversation`（薄壳 fork 包）、`client/ui-primitives`（CSS-in-JS 样式层 + 引用统一解析 / 渲染转换）、`context/dsh-context` 的 `reference` 出口（skill 引用后注入）；`session/ui-conversation-message-actions` 的 client 半在这一层之上替换 `conversation.chat.node` 的 `user` / `steering` 渲染、并覆盖 `conversation.composer.dock` 的 `stats` 行；动机、差异登记与回退条件见[债务 临时接管上游对话UI的client半](../packages/session/ui-conversation/.agents/debts/20260917-临时接管上游对话UI的client半.md) |
| profile 预设                 | —（无独立词汇）                                                                                                     | 两个 bundle：`preset/dsh-agent-preset`（注册 `coding` / `chat` 两个模式——每个模式一行 `@deepseek-ai/dsh-agent-preset`，行清单在 `tool/presets/*.ts`，通道与注入行同住该模式的 `isolate` 组）与 `preset/dsh-preset`（部署级 host 配置：pi-ai 路由、搜索后端、沙箱）                                                                                                                                                                                                                                                                                                                                               |
| 上下文注入                   | [`packages/context/dsh-context/.agents/CONTEXT.md`](../packages/context/dsh-context/.agents/CONTEXT.md)             | 给模型的注入形态与规则（规则块 / 内容块、id、同 id 覆盖、常驻与按需分层、回收）：`context/dsh-context` 一个包六个出口——`assembler`（唯一渲染者）、`agent-instructions`（工作区指令）、`skill-catalog`（skill 目录与 `skill` 工具）、`reference`（引用展开）、`tool-guidance`（工具用法分组）、`scope`（工具白名单与模式收口：目录与调用一起收口，可按模式关掉 instruction 与动态快照）；规则见[设计 上下文注入规则](../packages/context/dsh-context/.agents/designs/20260921-上下文注入规则.md)                                                                                                                  |
...
| web 搜索后端                 | —（无独立词汇）                                                                                                     | `web/dsh-web-search-ollama`：往 `ctx.web` 注册一个搜索后端（模型侧的 `web_search` 走 Ollama `/api/web_search`）；装配行与图片上限归 `preset/dsh-preset`                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **工具用法分组 → 上下文注入**：`context/dsh-context` 的 `tool-guidance` 出口 只提供分组内容，注入经
  `ctx.contextAssembler` 由 `context/dsh-context` 的 `assembler` 出口 送达（单向：通道不引用注入方）。决策见
  [ADR-提示词注入只有一个通道且按需内容走skill](../packages/context/dsh-context/.agents/adrs/20260921-提示词注入只有一个通道且按需内容走skill.md)。
...
- **对话 UI 接管 → 会话编辑**：`session/ui-conversation`（唯一的薄壳 fork 包）与 `client/ui-primitives` 提供 client 半的
  ...
  [债务 临时接管上游对话UI的client半](../packages/session/ui-conversation/.agents/debts/20260917-临时接管上游对话UI的client半.md)）；
  `context/dsh-context` 的 `reference` 出口 与 `preset/dsh-agent-preset` 的装配行一起工作（同住该模式的 `isolate` 组，
  通道才解析得到）。
